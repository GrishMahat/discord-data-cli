...
// Thin re-exports of the core library so existing `crate::<module>` paths
// inside the CLI keep resolving after the workspace split.
pub mod analyzer {
    pub use discord_data_cli_core::analyzer::*;
}
pub mod compare {
    pub use discord_data_cli_core::compare::*;
}
pub mod config {
    pub use discord_data_cli_core::config::*;
}
pub mod data {
    pub use discord_data_cli_core::data::*;
}
pub mod downloader {
    pub use discord_data_cli_core::downloader::*;
}
pub mod insights {
    pub use discord_data_cli_core::insights::*;
}
...
fn run_headless_analysis() -> Result<()> {
    ...
    if !app.config.package_path(&app.config_path, &app.id).exists() {
        bail!("No export configured. Run the TUI once to set up interactive.session.toml.");
    }
