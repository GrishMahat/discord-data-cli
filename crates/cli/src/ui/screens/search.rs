...
fn draw_type_filter(frame: &mut ratatui::Frame<'_>, app: &AppState, area: Rect) {
    ...
    for (label, filter) in tabs {
        spans.push(pill(label, app.search.type_filter == filter));
    }
    frame.render_widget(
    ...
    for (local_idx, result) in app.search.results[start..end].iter().enumerate() {
        ...
        let mut spans = vec![
            ...
            Span::styled(
                format!(
                    "{:<26} ",
                    discord_data_cli_core::data::utils::truncate_text(&result.title, 26)
                ),
                Style::default().fg(Color::Cyan),
            ),
            Span::styled(
                format!(
                    "{:<19} ",
                    discord_data_cli_core::data::utils::truncate_text(&result.timestamp, 19)
                ),
...
fn short_kind(kind: discord_data_cli_core::data::ChannelKind) -> &'static str {
    use discord_data_cli_core::data::ChannelKind;
    match kind {
        ChannelKind::Dm => "DM",
...
fn append_snippet_spans(spans: &mut Vec<Span<'static>>, content: &str, needle: &str) {
    ...
    let Some(pos) = match_pos else {
        ...
        spans.push(Span::styled(
            discord_data_cli_core::data::utils::truncate_text(content, WINDOW * 2),
