    ...
    if channels.is_empty() {
        ...
        frame.render_widget(
            ...
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .title(format!(" Channels: {} ", app.current_filter.label()))
...
fn draw_channel_list_pane(
    ...
    app: &AppState,
    channels: &[&discord_data_cli_core::data::MessageChannel],
    area: Rect,
) {
    ...
    for (local_idx, channel) in channels[start..end].iter().enumerate() {
        ...
        let title_width = title_width
            .min(area.width.saturating_sub(24) as usize)
            .max(8);
        let short_title =
            discord_data_cli_core::data::utils::truncate_text(&channel.title, title_width);
...
fn draw_preview_pane(frame: &mut ratatui::Frame<'_>, app: &AppState, area: Rect) {
    ...
    let base_title = match selected {
        Some(c) => format!(
            "Preview: {}",
            discord_data_cli_core::data::utils::truncate_text(&c.title, 42)
        ),
