...
fn draw_channel_types(frame: &mut ratatui::Frame<'_>, data: &analyzer::AnalysisData, area: Rect) {
    ...
    for (label, count) in &type_display {
        ...
        lines.push(Line::from(vec![
            ratatui::text::Span::styled(format!(" {}", bar), Style::default().fg(Color::Cyan)),
            ratatui::text::Span::styled(
                format!("  {:<9}", label),
                Style::default().fg(Color::White),
            ),
            ratatui::text::Span::styled(fmt_num(*count), Style::default().fg(Color::DarkGray)),
        ]));
    }
...
fn draw_data_overview(frame: &mut ratatui::Frame<'_>, data: &analyzer::AnalysisData, area: Rect) {
    let sections = [
        ...
        (
            "Support",
            data.folder_presence.get("support_tickets").copied(),
        ),
