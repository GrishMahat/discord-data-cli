...
pub(crate) fn draw_insights(frame: &mut ratatui::Frame<'_>, app: &AppState, area: Rect) {
    ...
    if !has_any {
        frame.render_widget(
            Paragraph::new(vec![
                Line::from(""),
                Line::styled("  No insights yet.", Style::default().fg(Color::DarkGray)),
                Line::from(""),
                Line::styled(
    ...
    let head = Line::from(vec![
        pill(
            &format!(" Payments {}", ins.billing.payments_total),
            Color::Green,
        ),
        Span::raw(" "),
        pill(
    ...
    if ins.billing.payments_total == 0 {
        ...
        if ins.billing.timeline.len() > 8 {
            note(
                &mut lines,
                &format!("   …and {} more", ins.billing.timeline.len() - 8),
            );
        }
    }
    ...
    if ins.voice.connections == 0 && ins.voice.disconnects == 0 {
        ...
        kv(
            ...
            &format!(
                "{} lost of {} sent ({})",
                ins.voice.packets_lost, ins.voice.packets_sent, loss
            ),
        );
        kv(
            &mut lines,
    ...
    for (i, contact) in ins.social.top_contacts.iter().enumerate().take(6) {
        ...
        lines.push(Line::from(vec![
            ...
            Span::styled(format!("{:<26} ", truncate(&contact.name, 26)), white()),
    ...
    if !ins.servers_engaged.is_empty() {
        ...
        for (i, server) in ins.servers_engaged.iter().enumerate().take(8) {
            lines.push(Line::from(vec![
                Span::styled(format!("   {}. ", i + 1), dim()),
                Span::styled(format!("{:<30} ", truncate(&server.name, 30)), white()),
    ...
    kv(
        &mut lines,
        "Email verified",
        yes_no(ins.privacy.email_verified),
    );
...
fn truncate(s: &str, n: usize) -> String {
    discord_data_cli_core::data::utils::truncate_text(s, n)
}
