...
pub(crate) use data::{ActivityEventPreview, ChannelKind, MessageChannel, SupportTicketView};
...
impl AppState {
    ...
    pub(crate) fn compare_snapshots(
        &self,
    ) -> Vec<(String, discord_data_cli_core::compare::Snapshot)> {
        let results_dir = self.config.results_path(&self.config_path, &self.id);
        discord_data_cli_core::compare::list_snapshots(&results_dir)
    }
}
