    ...
    thread::spawn(move || {
        ...
        if results_dir.exists() {
            ...
            for cat in cats {
                ...
                if cat_dir.is_dir()
                    && let Ok(entries) = fs::read_dir(cat_dir)
                {
...
fn start_download(app: &mut AppState, links: Vec<String>) {
    ...
    thread::spawn(move || {
        let tx2 = tx.clone();
        let result = downloader::download_attachments(&results_dir, links, abort, move |p| {
...
pub(crate) fn poll_channel_preview(app: &mut AppState) {
        ...
        && app.channel_preview_for.as_deref() != Some(channel.dir_name.as_str())
    {
        ...
        thread::spawn(move || {
            let result =
                data::load_message_preview(&worker_channel, limit).map_err(|e| e.to_string());
...
pub(crate) fn ensure_channels_loaded(app: &mut AppState) {
    ...
    thread::spawn(move || {
        let result =
            data::load_channels(&package_dir, &aliases, &cached_counts).map_err(|e| e.to_string());
...
pub(crate) fn start_message_search(app: &mut AppState) {
    ...
    let (tx, rx) = mpsc::channel();
    thread::spawn(move || {
        use discord_data_cli_core::data::utils::{
        ...
        for (i, ch) in channels.iter().enumerate() {
            ...
            let _ = stream_records(&ch.messages_path, &mut |record| {
                ...
                let timestamp =
                    pick_str(record, &["Timestamp", "timestamp", "timestamp_ms", "date"])
                        .unwrap_or("unknown")
                        .to_owned();
