...
pub struct AttachmentHashIndex {
...
pub fn load_hash_index(path: &Path) -> Result<AttachmentHashIndex> {
    if !path.is_file() {
...
pub fn save_hash_index(path: &Path, index: &AttachmentHashIndex) -> Result<()> {
...
pub fn canonical_attachment_key(url: &str) -> String {
