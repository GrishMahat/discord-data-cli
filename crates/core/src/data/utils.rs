...
pub fn read_records_json_or_ndjson(path: &Path) -> Result<Vec<Value>> {
    let file = File::open(path).with_context(|| format!("failed to open {}", path.display()))?;
    let reader = BufReader::new(file);
...
pub fn find_file_case_insensitive(dir: &Path, file_name: &str) -> Result<Option<PathBuf>> {
...
pub fn resolve_optional_subdir(package_dir: &Path, names: &[String]) -> Result<Option<PathBuf>> {
...
pub fn pick_str<'a>(record: &'a Value, keys: &[&str]) -> Option<&'a str> {
    for key in keys {
...
pub fn pick_plain_string(record: &Value, keys: &[&str]) -> Option<String> {
    for key in keys {
...
pub fn pick_timestamp_month(record: &Value, keys: &[&str]) -> Option<String> {
    pick_plain_string(record, keys).and_then(|text| parse_year_month(&text))
}

pub fn parse_year_month(text: &str) -> Option<String> {
...
pub fn extract_message_content(record: &Value) -> String {
...
pub fn extract_attachment_urls(record: &Value) -> Vec<String> {
...
pub fn attachment_value_to_urls(value: &Value) -> Vec<String> {
    match value {
...
pub fn channel_title(channel: Option<&Value>, fallback_id: &str) -> String {
...
pub fn stream_records(path: &Path, f: &mut dyn FnMut(&Value)) -> Result<usize> {
    let file = File::open(path).with_context(|| format!("failed to open {}", path.display()))?;
    let mut reader = BufReader::new(file);
...
pub fn read_records_tail(path: &Path, n: usize) -> Result<Vec<Value>> {
    if n == 0 {
        return Ok(Vec::new());
    }
...
pub fn read_ndjson_tail(path: &Path, n: usize, max_tail_bytes: u64) -> Result<Vec<Value>> {
...
pub fn normalize_inline(text: &str) -> String {
    text.split_whitespace().collect::<Vec<_>>().join(" ")
}

pub fn parse_date_key(text: &str) -> Option<String> {
    let bytes = text.as_bytes();
    if bytes.len() < 10 {
        return None;
    }
...
pub fn normalize_sort_key(timestamp: &str) -> String {
