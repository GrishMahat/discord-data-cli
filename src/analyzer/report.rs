...
pub fn read_data(results_dir: &Path) -> Result<Option<AnalysisData>> {
    ...
    let parsed: AnalysisData = serde_json::from_reader(reader).with_context(|| "failed to parse data.json".to_owned())?;
    Ok(Some(parsed))
}
...
pub fn generate_markdown_report(stats: &AnalysisData) -> String {
    ...
    if let Some(user) = &stats.account.username { out.push_str(&format!("**Account:** {}\n", user)); }
    out.push_str("\n## Overview\n");
