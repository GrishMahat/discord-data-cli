...
pub fn analyze_account(account_dir: &Path, stats: &mut AnalysisData) -> Result<()> {
    ...
    if !user_path.exists() {
        stats.warnings.push("Account/user.json missing; user profile summary skipped.".to_owned());
        return Ok(());
    }
    let value = read_json_value(&user_path)?;
    stats.account.user_id = value.get("id").and_then(value_to_plain_string);
    stats.account.username = value.get("global_name").and_then(value_to_plain_string)
...
pub fn analyze_servers(servers_dir: Option<&Path>, stats: &mut AnalysisData) -> Result<()> {
    let Some(servers_dir) = servers_dir else { return Ok(()); };
    ...
    for entry in fs::read_dir(servers_dir)? {
        ...
        if audit_path.is_file() { stats.servers.audit_log_entries += count_json_records(&audit_path)?; }
    }
    Ok(())
}
...
pub fn analyze_support_tickets(support_dir: Option<&Path>, stats: &mut AnalysisData) -> Result<()> {
    let Some(support_dir) = support_dir else { return Ok(()); };
    let Some(tickets_path) = find_file_case_insensitive(support_dir, "tickets.json")? else { return Ok(()); };
    let tickets_value = read_json_value(&tickets_path)?;
    match tickets_value {
        Value::Object(map) => { for (_, value) in map { summarize_ticket(&value, stats); } }
        Value::Array(items) => { for value in items { summarize_ticket(&value, stats); } }
        _ => {}
    }
...
pub fn summarize_ticket(value: &Value, stats: &mut AnalysisData) {
    ...
    let status = pick_plain_string(value, &["status", "ticket_status", "state"]).unwrap_or_else(|| "unknown".to_owned());
    ...
    if let Some(created_month) = pick_timestamp_month(value, &["created_at", "createdAt", "created", "opened_at", "openedAt", "date", "timestamp"]) {
    ...
    if let Some(Value::Array(comments)) = value.get("comments") {
        ...
        if comment_count > 0 { stats.support_tickets.tickets_with_comments += 1; }
        for comment in comments {
            if let Some(month) = pick_timestamp_month(comment, &["created_at", "createdAt", "date", "timestamp", "updated_at", "updatedAt"]) {
                increment_counter(&mut stats.support_tickets.activity_by_month, month, 1);
            }
        }
    }
    if let Some(month) = pick_timestamp_month(value, &["updated_at", "updatedAt", "last_activity_at", "lastActivityAt", "closed_at", "closedAt", "resolved_at", "resolvedAt"]) {
        increment_counter(&mut stats.support_tickets.activity_by_month, month, 1);
    }
}
...
pub fn analyze_activity<F>(activity_dir: Option<&Path>, stats: &mut AnalysisData, mut on_progress: F) -> Result<()>
where F: FnMut(f32, String) {
    let Some(activity_dir) = activity_dir else { return Ok(()); };
    let mut files: Vec<PathBuf> = WalkDir::new(activity_dir).into_iter().filter_map(|e| e.ok()).filter(|e| e.file_type().is_file())
        .map(|e| e.into_path()).filter(|p| p.extension().map(|ext| ext.to_string_lossy().eq_ignore_ascii_case("json")).unwrap_or(false)).collect();
    files.sort();
    ...
    for path in files {
        ...
        let rel_path = path.strip_prefix(activity_dir).unwrap_or(&path).to_string_lossy().to_string();
        if let Some(cached) = stats.activity_cache.get(&rel_path) {
            if cached.mtime_ms == mtime {
                ...
                for (et, c) in &cached.stats.event_types { increment_counter(&mut stats.activity.by_event_type, et, *c); }
        ...
        let short_name = path.file_name().and_then(|s| s.to_str()).unwrap_or("unknown.json").to_owned();
        tasks.push(ActivityFileTask { index, path: path.clone(), size, short_name });
        tasks_paths.push((index, rel_path, mtime));
    }
    if tasks.is_empty() { stats.activity_cache = next_activity_cache; return Ok(()); }
    ...
    let worker_count = thread::available_parallelism().map(|n| n.get()).unwrap_or(4).min(total_tasks).max(1);
    ...
    for _ in 0..worker_count {
        ...
        handles.push(thread::spawn(move || {
            while let Some(task) = { let mut q = queue.lock().unwrap(); q.pop_front() } {
                match process_activity_file(&task, &worker_tx) {
                    Ok(fstats) => { let _ = worker_tx.send(ActivityWorkerEvent::Finished { file_index: task.index, stats: fstats }); }
                    Err(err) => { let _ = worker_tx.send(ActivityWorkerEvent::Failed { _file_index: task.index, error: err.to_string() }); }
                }
            }
        }));
    }
    ...
    while finished < total_tasks {
        match rx.recv()? {
            ActivityWorkerEvent::Progress { file_index, bytes_read } => {
                ...
                on_progress(total_bytes_read as f32 / total_bytes as f32, format!("file {}/{}: {}", file_index + 1, total_tasks, file_names[file_index]));
            }
            ActivityWorkerEvent::Finished { file_index, stats: fstats } => {
                ...
                for (et, c) in &fstats.event_types { increment_counter(&mut stats.activity.by_event_type, et, *c); }
                if let Some((_, rel_path, mtime)) = tasks_paths.iter().find(|(idx, _, _)| *idx == file_index) {
                    next_activity_cache.insert(rel_path.clone(), ActivityFileCache { mtime_ms: *mtime, stats: fstats });
                }
                finished += 1;
                on_progress(total_bytes_read as f32 / total_bytes as f32, format!("file {}/{}: {} complete", file_index + 1, total_tasks, file_names[file_index]));
            }
            ActivityWorkerEvent::Failed { _file_index, error } => { finished += 1; stats.warnings.push(error); }
        }
    }
...
// Pray for our RAM.
fn process_activity_file(task: &ActivityFileTask, tx: &Sender<ActivityWorkerEvent>) -> Result<ActivityFileStats> {
    ...
    while reader.read_until(b'\n', &mut line)? > 0 {
        ...
        if bytes_read >= next_report {
            let _ = tx.send(ActivityWorkerEvent::Progress { file_index: task.index, bytes_read });
        ...
        if !trimmed.is_empty() {
            ...
            if let Ok(value) = serde_json::from_slice::<ActivityEventLine>(trimmed) {
                increment_counter(&mut fstats.event_types, value.event_type.unwrap_or(Cow::Borrowed("unknown")), 1);
            } else { fstats.parse_errors += 1; }
        }
        line.clear();
    }
    Ok(fstats)
...
struct ActivityEventLine<'a> { #[serde(borrow, default)] event_type: Option<Cow<'a, str>> }

struct ActivityFileTask { index: usize, path: PathBuf, size: u64, short_name: String }

enum ActivityWorkerEvent {
    Progress { file_index: usize, bytes_read: u64 },
    Finished { file_index: usize, stats: ActivityFileStats },
    Failed { _file_index: usize, error: String },
}
...
pub fn analyze_messages(messages_dir: Option<&Path>, stats: &mut AnalysisData) -> Result<()> {
    ...
    let mut dirs: Vec<PathBuf> = fs::read_dir(messages_dir)?.filter_map(|e| e.ok()).map(|e| e.path()).filter(|p| p.is_dir()).collect();
    dirs.sort();
    ...
    for dir in dirs {
        let id = dir.file_name().and_then(|n| n.to_str()).unwrap_or("unknown").to_owned();
        let m_path = dir.join("messages.json");
        let c_path = dir.join("channel.json");
        if !m_path.is_file() { continue; }
        ...
        if let Some(cached) = stats.channels_cache.get(&id) {
            if cached.mtime_messages == mt_m && cached.mtime_channel == mt_c {
                ...
                for (w, c) in &cached.word_frequency { *total_word_freq.entry(w.clone()).or_insert(0) += c; }
                for (ch, c) in &cached.content.character_frequency { *total_char_freq.entry(*ch).or_insert(0) += c; }
                ch_counts.push((cached.channel_title.clone(), cached.message_count));
                next_cache.insert(id, cached.clone());
                continue;
            }
        }
        tasks.push(ChannelTask { id, messages_path: m_path, channel_path: c_path, mtime_messages: mt_m, mtime_channel: mt_c });
    }

        // We have multiple workers parsing thousands of files. My CPU temperature is approaching that of the sun.
        if !tasks.is_empty() {
        let worker_count = thread::available_parallelism().map(|n| n.get()).unwrap_or(4).min(tasks.len()).max(1);
        let queue = Arc::new(Mutex::new(VecDeque::from(tasks)));
        let (tx, rx) = mpsc::channel::<ChannelWorkerEvent>();
        for _ in 0..worker_count {
            let q = Arc::clone(&queue);
            let wtx = tx.clone();
            let ere = emoji_re.clone(); let hre = hour_re.clone(); let dre = date_re.clone(); let wre = word_re.clone();
            thread::spawn(move || {
                while let Some(task) = { let mut g = q.lock().unwrap(); g.pop_front() } {
                    let mut cstats = ChannelAnalysisCache { mtime_messages: task.mtime_messages, mtime_channel: task.mtime_channel, ..Default::default() };
                    if let Ok(val) = read_json_value(&task.channel_path) {
                        cstats.channel_type = val.get("type").and_then(value_to_plain_string).unwrap_or_else(|| "unknown".to_owned());
                        cstats.channel_title = channel_title(Some(&val), &task.id);
                    } else { cstats.channel_title = task.id.clone(); }
                    ...
                    if let Ok(records) = read_records_json_or_ndjson(&task.messages_path) {
                        for rec in records {
                            ...
                            if let Some(ts) = pick_str(&rec, &["Timestamp", "timestamp", "timestamp_ms", "date"]) {
                                if let Some(caps) = hre.captures(ts) { if let Ok(hr) = caps[1].parse::<u32>() { *cstats.temporal.by_hour.entry(hr).or_insert(0) += 1; } }
                                if let Some(caps) = dre.captures(ts) {
                                    let (y, m, d) = (caps[1].parse::<u32>().unwrap_or(0), caps[2].parse::<u32>().unwrap_or(0), caps[3].parse::<u32>().unwrap_or(0));
                                    if (1..=12).contains(&m) { *cstats.temporal.by_month.entry(m).or_insert(0) += 1; }
                                    if y >= 1 && m >= 1 && d >= 1 { *cstats.temporal.by_day_of_week.entry(day_of_week(y,m,d)).or_insert(0) += 1; }
                                    let ds = format!("{y:04}-{m:02}-{d:02}");
                                    if cstats.temporal.first_message_date.as_deref().is_none_or(|f| ds < f.to_owned()) { cstats.temporal.first_message_date = Some(ds.clone()); }
                                    if cstats.temporal.last_message_date.as_deref().is_none_or(|l| ds > l.to_owned()) { cstats.temporal.last_message_date = Some(ds); }
                                }
                            }
                            if !content.is_empty() {
                                ...
                                cstats.content.emoji_custom += ere.find_iter(&content).count() as u64;
                                for g in content.graphemes(true) { if emojis::get(g).is_some() { cstats.content.emoji_unicode += 1; } }
                                for ch in content.chars() { *cstats.content.character_frequency.entry(ch).or_insert(0) += 1; }
                                for mat in wre.find_iter(&content.to_ascii_lowercase()) {
                                    if !is_stop_word(mat.as_str()) { *cstats.word_frequency.entry(mat.as_str().to_owned()).or_insert(0) += 1; }
                                }
                            }
                            for url in attachments { if url.starts_with("https://cdn.discordapp.com/attachments/") { cstats.attachment_links.push(url); } }
                            cstats.attachment_count = cstats.attachment_links.len() as u64;
                        }
                        let _ = wtx.send(ChannelWorkerEvent::Finished { id: task.id, stats: cstats });
                    } else { let _ = wtx.send(ChannelWorkerEvent::Failed { id: task.id, error: "Read failed".to_owned() }); }
                }
            });
        }
        drop(tx);
        while let Ok(event) = rx.recv() {
            match event {
                ChannelWorkerEvent::Finished { id, stats: c_entry } => {
                    stats.messages.channels += 1; stats.messages.total += c_entry.message_count;
                    stats.messages.with_content += c_entry.messages_with_content;
                    stats.messages.with_attachments += c_entry.attachment_count;
                    stats.messages.attachment_links.extend(c_entry.attachment_links.clone());
                    increment_counter(&mut stats.messages.by_channel_type, &c_entry.channel_type, 1);
                    stats.messages.temporal.merge(&c_entry.temporal);
                    stats.messages.content.merge(&c_entry.content);
                    for (w, c) in &c_entry.word_frequency { *total_word_freq.entry(w.clone()).or_insert(0) += c; }
                    for (ch, c) in &c_entry.content.character_frequency { *total_char_freq.entry(*ch).or_insert(0) += c; }
                    ch_counts.push((c_entry.channel_title.clone(), c_entry.message_count));
                    next_cache.insert(id, c_entry);
                }
                ChannelWorkerEvent::Failed { id, error } => { stats.warnings.push(format!("Channel {id}: {error}")); }
            }
        }
    }
    stats.channels_cache = next_cache;
    stats.messages.content.distinct_characters = total_char_freq.len();
    stats.messages.content.avg_length_chars = if stats.messages.with_content > 0 { stats.messages.content.total_chars as f64 / stats.messages.with_content as f64 } else { 0.0 };
...
pub fn analyze_activities(activities_dir: Option<&Path>, stats: &mut AnalysisData) -> Result<()> {
    let Some(activities_dir) = activities_dir else { return Ok(()); };
    for entry in WalkDir::new(activities_dir).into_iter().filter_map(|e| e.ok()).filter(|e| e.file_type().is_file()) {
        stats.activities.files += 1;
        let name = entry.file_name().to_string_lossy().to_ascii_lowercase();
        if name == "favorite_games.json" {
            if let Ok(v) = read_json_value(entry.path()) {
                stats.activities.favorite_games = v.get("favorite_games").and_then(|v| v.as_u64()).or(stats.activities.favorite_games);
            }
        } else if name == "preferences.json" {
            if let Ok(Value::Array(items)) = read_json_value(entry.path()) { stats.activities.preferences_entries += items.len() as u64; }
        } else if name == "user_data.json" {
            if let Ok(Value::Object(map)) = read_json_value(entry.path()) { stats.activities.user_data_apps += map.len() as u64; }
        }
    }
    Ok(())
}
...
pub fn analyze_programs(programs_dir: Option<&Path>, stats: &mut AnalysisData) -> Result<()> {
    let Some(programs_dir) = programs_dir else { return Ok(()); };
    stats.programs.files = WalkDir::new(programs_dir).into_iter().filter_map(|e| e.ok()).filter(|e| e.file_type().is_file()).count() as u64;
    Ok(())
}

struct ChannelTask { id: String, messages_path: PathBuf, channel_path: PathBuf, mtime_messages: u64, mtime_channel: u64 }
enum ChannelWorkerEvent { Finished { id: String, stats: ChannelAnalysisCache }, Failed { id: String, error: String } }
...
pub fn get_mtime_ms(path: &Path) -> u64 {
    fs::metadata(path).and_then(|m| m.modified()).ok().and_then(|t| t.duration_since(UNIX_EPOCH).ok()).map(|d| d.as_millis() as u64).unwrap_or(0)
}
