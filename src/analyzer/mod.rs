...
// It's like a loading bar, but with more existential dread (how much data is there?!).
#[derive(Debug, Clone)]
    ...
    F: FnMut(AnalysisProgress),
{
    ...
    let mut stats = AnalysisData {
        meta: structs::Meta { tool_version: env!("CARGO_PKG_VERSION").to_owned(), analyzed_at: utc_now_iso8601(), package_directory: package_dir.display().to_string(), results_directory: results_dir.display().to_string() },
        ...
        channels_cache: existing.as_ref().map(|d| d.channels_cache.clone()).unwrap_or_default(),
        activity_cache: existing.as_ref().map(|d| d.activity_cache.clone()).unwrap_or_default(),
    ...
    emit(&mut on_progress, step_frac(2.0), AnalysisStep::Messages, "Analyzing messages...");
    workers::analyze_messages(source_dirs.messages.as_deref(), &mut stats)?;
    emit(&mut on_progress, step_frac(3.0), AnalysisStep::Servers, "Analyzing servers...");
    workers::analyze_servers(source_dirs.servers.as_deref(), &mut stats)?;
    emit(&mut on_progress, step_frac(4.0), AnalysisStep::Support, "Analyzing support tickets...");
    workers::analyze_support_tickets(source_dirs.support_tickets.as_deref(), &mut stats)?;
    
    emit(&mut on_progress, step_frac(5.0), AnalysisStep::Activity, "Analyzing activity events...");
    check_abort()?;
    workers::analyze_activity(source_dirs.activity.as_deref(), &mut stats, |f, d| {
        emit(&mut on_progress, step_frac(5.0 + f), AnalysisStep::Activity, format!("Analyzing activity events... {d}"));
    })?;

    emit(&mut on_progress, step_frac(6.0), AnalysisStep::Activities, "Analyzing activities...");
    workers::analyze_activities(source_dirs.activities.as_deref(), &mut stats)?;
    emit(&mut on_progress, step_frac(7.0), AnalysisStep::Programs, "Analyzing programs...");
    workers::analyze_programs(source_dirs.programs.as_deref(), &mut stats)?;

    emit(&mut on_progress, step_frac(8.0), AnalysisStep::Writing, "Writing results...");
    let data_path = results_dir.join("data.json");
    fs::write(&data_path, serde_json::to_string_pretty(&stats)?)?;
    let _ = fs::write(results_dir.join("report.md"), report::generate_markdown_report(&stats));

    emit(&mut on_progress, 1.0, AnalysisStep::Complete, "Analysis complete.");
    Ok(stats)
}
...
fn utc_now_iso8601() -> String {
    ...
    format!("{year:04}-{month:02}-{day:02}T{:02}:{:02}:{:02}Z", (now/3600)%24, (now/60)%60, now%60)
}
...
fn days_to_ymd(mut days: u64) -> (u64, u64, u64) {
    ...
    let mut month = 1;
    for &d in &mdays { if days < d { break; } days -= d; month += 1; }
    (year, month, days + 1)
}
