...
{
    if links.is_empty() {
        on_progress(DownloadProgress { fraction: 1.0, label: "No attachments to download.".to_owned() });
        return Ok(());
    }
    ...
    for category in categories { fs::create_dir_all(results_dir.join(category))?; }
    ...
    for (i, (url, dedupe_key)) in unique_links.into_iter().enumerate() {
        ...
        tasks.push(worker::DownloadTask { url, output_path, temp_path });
    }
    ...
    for _ in 0..num_workers {
        ...
        thread::spawn(move || {
            loop {
                ...
                if task.output_path.exists() && task.output_path.metadata().map(|m| m.len()).unwrap_or(0) > 0 {
                ...
                for _attempt in 1..=3 {
                    ...
                    let content_hash = match worker::download_to_temp_and_hash(&task.url, &task.temp_path) {
                        Ok(hash) => hash,
                    ...
                    let mut claimed_hash = {
                        ...
                        if in_flight.contains(&content_hash) { false } else { in_flight.insert(content_hash.clone()); true }
                    };
                    ...
                    if !claimed_hash {
                        for _ in 0..40 {
                            ...
                            let exists_now = {
                                ...
                                index.hashes.get(&content_hash).map(|p| Path::new(p).exists()).unwrap_or(false)
                            };
                            if exists_now {
                                let _ = fs::remove_file(&task.temp_path);
                            ...
                            if can_claim_now { claimed_hash = true; break; }
                        }
                        if handled { break; }
                        if !claimed_hash {
                            let _ = fs::remove_file(&task.temp_path);
                    ...
                    match worker::finalize_temp_file(&task.temp_path, &task.output_path) {
                        Ok(()) => {
                            { let mut index = hash_index.lock().unwrap(); index.hashes.insert(content_hash.clone(), output_path_text); }
                            { let mut in_flight = in_flight_hashes.lock().unwrap(); in_flight.remove(&content_hash); }
                            handled = true;
                            saved_new_file = true;
                            break;
                        }
                ...
                if saved_new_file { *downloaded.lock().unwrap() += 1; } else if !handled { *failed.lock().unwrap() += 1; }
                let _ = tx.send(());
            }
        });
    }
    ...
    while let Ok(()) = rx.recv() {
        ...
        on_progress(DownloadProgress {
            ...
            label: format!("Downloading: {}/{} ({} saved, {} existing, {} dup-content, {} failed, {} dup-url)", completed, total, d, s, dc, f, duplicates_ignored_url),
        });
    }
    ...
    on_progress(DownloadProgress {
        fraction: 1.0,
        label: format!("Download finished. {} saved, {} existing, {} dup-content, {} failed, {} dup-url.", d, s, dc, f, duplicates_ignored_url),
    });
...
fn attachment_basename(url_or_path: &str) -> &str {
    ...
    if segment.is_empty() { "attachment" } else { segment }
}

fn guess_category(url: &str) -> &'static str {
    let lower = url.to_ascii_lowercase();
    if lower.contains(".mp3") || lower.contains(".wav") || lower.contains(".m4a") { "audios" }
    else if lower.contains(".doc") || lower.contains(".pdf") { "docs" }
    else if lower.contains(".jpg") || lower.contains(".jpeg") || lower.contains(".png") || lower.contains(".gif") || lower.contains(".webp") { "imgs" }
    else if lower.contains(".txt") { "txts" }
    else if lower.contains(".py") || lower.contains(".js") || lower.contains(".html") || lower.contains(".css") || lower.contains(".json") { "codes" }
    else if lower.contains(".exe") || lower.contains(".msi") { "exes" }
    else if lower.contains(".mp4") || lower.contains(".mov") || lower.contains(".webm") || lower.contains(".mkv") { "vids" }
    else if lower.contains(".zip") || lower.contains(".rar") || lower.contains(".7z") || lower.contains(".tar") || lower.contains(".gz") { "zips" }
    else { "unknowns" }
}
...
mod tests {
    ...
    impl TestServer {
        fn start() -> Result<Self> {
            ...
            Ok(Self { base_url: format!("http://{addr}"), stop, handle: Some(handle) })
        }
    }
    ...
    impl Drop for TestServer {
        fn drop(&mut self) {
            ...
            let _ = TcpStream::connect(self.base_url.strip_prefix("http://").unwrap_or("127.0.0.1:1"));
    ...
    fn make_temp_results_dir() -> Result<PathBuf> {
        ...
        let path = std::env::temp_dir().join(format!("discord-data-analyzer-test-{}-{}", std::process::id(), ts));
    ...
    fn count_saved_attachments(results_dir: &Path) -> Result<usize> {
        ...
        for entry in fs::read_dir(results_dir)? {
            ...
            if name == ".tmp_downloads" { continue; }
            for file in fs::read_dir(&path)? {
                let fp = file?.path();
                if fp.is_file() && fp.file_name().and_then(|n| n.to_str()).unwrap_or("").starts_with("attachment_") { count += 1; }
            }
        }
        Ok(count)
    }
    ...
    fn dedupes_url_and_content_across_runs() -> Result<()> {
        ...
        let first_final_label = Arc::new(Mutex::new(String::new()));
        {
            ...
            download_attachments(&results_dir, links.clone(), move |progress| {
                if let Ok(mut label) = label_ref.lock() { *label = progress.label; }
            })?;
        }
