...
use crate::analyzer::structs::{
    BillingInsights, ContactStat, EventAggregate, Insights, PaymentEntry, PrivacyInsights,
    ServerEngagement, SocialInsights, EVENT_AGGREGATE_VERSION,
};
use crate::data::utils::{pick_str, read_json_value, value_to_plain_string};

/// Billing + privacy posture from the Account folder.
pub(crate) fn compute_account(
...
fn compute_billing_and_privacy(account_dir: Option<&Path>, stats: &mut crate::analyzer::AnalysisData) -> Result<()> {
    ...
    if let Ok(user) = read_json_value(&account_dir.join("user.json")) {
        privacy.email_verified = user.get("verified").and_then(Value::as_bool).unwrap_or(false);
        ...
        read_json_value(path)
            ...
            .and_then(|v| {
                v.get("record_count")
                    ...
                    .or_else(|| v.get("records").and_then(Value::as_array).map(|a| a.len() as u64))
            })
            .unwrap_or(0)
    };

    privacy.payment_sources = record_count(&exports_dir.join("discord_billing/payment_sources.json"));
    ...
    if let Ok(payments) = read_json_value(&payments_path) {
        ...
        for p in &entries {
            *billing.totals_by_currency.entry(p.currency.clone()).or_insert(0) += p.amount;
            *billing.by_gateway.entry(p.gateway).or_insert(0) += 1;
        }
        entries.sort_by(|a, b| b.date.cmp(&a.date).then_with(|| a.description.cmp(&b.description)));
...
fn sort_contacts(v: &mut [ContactStat]) {
    v.sort_by(|a, b| b.messages.cmp(&a.messages).then_with(|| a.name.cmp(&b.name)));
}
...
pub(crate) fn rank_servers(
    ...
    let mut ranked: Vec<ServerEngagement> = aggregate
        ...
        .map(|(id, events)| ServerEngagement {
            name: names.get(id).cloned().unwrap_or_else(|| format!("server…{}", &id[id.len().saturating_sub(6)..])),
            events: *events,
...
pub(crate) fn versioned_signature(files: &[PathBuf]) -> u64 {
...
pub(crate) fn compute_activity<F>(
    ...
    match event_type.as_str() {
        ...
        "voice_disconnect" => {
            ...
            if let Some(reason) = pick_str(rec, &["reason"]).filter(|s| !s.is_empty()) {
                ...
                *agg.disconnect_reasons.entry("(unspecified)".to_owned()).or_insert(0) += 1;
            }
            ...
            agg.packets_lost += num_u64(rec.get("packets_received_lost"))
                + num_u64(rec.get("packets_sent_lost"));
            ...
            for key in [
                ...
                if let Some(ms) = num_f64(rec.get(key)) {
                    ...
                    *agg.connection_type_ms.entry(label.to_owned()).or_insert(0.0) += ms;
                }
            }
        }
...
pub(crate) fn day_ordinal(date: &str) -> Option<i64> {
...
pub(crate) fn write_summaries(results_dir: &Path, insights: &Insights) -> Result<()> {
...
fn str_or(v: Option<&Value>, fallback: &str) -> String {
    v.and_then(value_to_plain_string).unwrap_or_else(|| fallback.to_owned())
}
...
pub(crate) fn activity_event_files(activity_dir: &Path) -> Vec<PathBuf> {
...
pub(crate) fn signature_for(files: &[PathBuf]) -> u64 {
