...
impl Snapshot {
    pub(crate) fn from_data(data: &AnalysisData) -> Self {
        ...
        Self {
            ...
            top_contact: ins.social.top_contacts.first().map(|c| (c.name.clone(), c.messages)),
...
pub(crate) fn write_snapshot(results_dir: &Path, data: &AnalysisData) -> Result<()> {
...
pub(crate) fn list_snapshots(results_dir: &Path) -> Vec<(String, Snapshot)> {
    let dir = snapshots_dir(results_dir);
...
pub(crate) fn diff_snapshots(old: &Snapshot, new: &Snapshot) -> Vec<DiffRow> {
    ...
    push_count("Voice connections", old.voice_connections, new.voice_connections);
    push_count(
        "Voice minutes",
        old.voice_minutes,
        new.voice_minutes,
