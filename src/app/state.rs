...
pub(crate) fn home_item_disabled_reason(app: &AppState, idx: usize) -> Option<String> {
    ...
    match idx {
        2 if !folder_available(data, "support_tickets") => Some("Support Tickets is disabled: source data was not included in this export.".to_owned()),
        3 if !folder_available(data, "activity") => Some("Activity Explorer is disabled: activity data was not included in this export.".to_owned()),
        4 if !folder_available(data, "messages") => Some("Attachment Downloader is disabled: messages data was not included in this export.".to_owned()),
        5 if !folder_available(data, "messages") => Some("Gallery is disabled: messages data was not included in this export.".to_owned()),
        6..=10 if !folder_available(data, "messages") => Some("Messages features are disabled: messages data was not included in this export.".to_owned()),
        _ => None,
    }
}
...
pub(crate) fn screen_disabled_reason(app: &AppState, screen: Screen) -> Option<String> {
    match screen {
        Screen::Home | Screen::Setup | Screen::Settings | Screen::Analyzing | Screen::Downloading => return None,
    ...
    match screen {
        ...
        Screen::SupportActivity if !folder_available(data, "support_tickets") => Some("Support is disabled: source data was not included in this export.".to_owned()),
        Screen::Activity if !folder_available(data, "activity") => Some("Activity is disabled: source data was not included in this export.".to_owned()),
        Screen::ChannelList | Screen::Gallery if !folder_available(data, "messages") => Some("Channels is disabled: messages data was not included in this export.".to_owned()),
        _ => None,
    }
}
...
pub(crate) fn setup_submit_step(app: &mut AppState) -> Result<()> {
    match app.setup.step {
        SetupStep::ExportPath => {
            ...
            if raw.is_empty() { bail!("Export path is required."); }
            let export_dir = to_absolute(PathBuf::from(raw))?;
            if !export_dir.is_dir() { bail!("Export path not found: {}", export_dir.display()); }
        ...
        SetupStep::ResultsPath => {
            let raw = app.setup.input.trim();
            let selected = if raw.is_empty() { PathBuf::from(&app.setup.results_path) } else { to_absolute(PathBuf::from(raw))? };
            if selected.exists() && !selected.is_dir() { bail!("Results path exists but is not a directory: {}", selected.display()); }
...
fn apply_setup(app: &mut AppState) -> Result<()> {
    ...
    if !package_dir.is_dir() { bail!("Export path not found: {}", package_dir.display()); }

    let results_raw = app.setup.results_path.trim();
    let results_dir = to_absolute(PathBuf::from(results_raw))?;
    if !results_dir.exists() {
        fs::create_dir_all(&results_dir).with_context(|| format!("failed to create {}", results_dir.display()))?;
    }
...
fn open_setup_with_current_values(app: &mut AppState) {
    app.setup.export_path = app.config.package_path(&app.config_path, &app.id).display().to_string();
    app.setup.results_path = app.config.results_path(&app.config_path, &app.id).display().to_string();
...
pub(crate) fn open_support_activity(app: &mut AppState) -> Result<()> {
    try_load_existing_data(app);
    if app.support_tickets.is_none() { start_support_tickets_load(app); }
...
pub(crate) fn open_activity(app: &mut AppState) -> Result<()> {
    try_load_existing_data(app);
    if app.activity_events.is_none() { start_activity_events_load(app); }
    app.screen = Screen::Activity;
    Ok(())
}

pub(crate) fn start_support_tickets_load(app: &mut AppState) {
    if app.support_activity_loading { return; }
...
pub(crate) fn start_activity_events_load(app: &mut AppState) {
    if app.activity_loading { return; }
    ...
    let aliases = app.config.source_aliases.clone();
    thread::spawn(move || {
        let result = data::load_recent_activity_events(&package_dir, &aliases, 250).map_err(|e| e.to_string());
        ...
        loop {
            match rx.try_recv() {
                Ok(SupportActivityEvent::TicketsFinished(result)) => {
                    ...
                    if let Ok(tickets) = result { app.support_tickets = Some(tickets); }
                }
                Ok(SupportActivityEvent::ActivityFinished(result)) => {
                    app.activity_loading = false;
                    if let Ok(events) = result { app.activity_events = Some(events); }
                }
                Err(mpsc::TryRecvError::Empty) => break,
                Err(mpsc::TryRecvError::Disconnected) => { closed = true; break; }
            }
        }
        if closed || (!app.support_activity_loading && !app.activity_loading) { app.support_activity_rx = None; }
    }
}
...
pub(crate) fn open_gallery(app: &mut AppState) -> Result<()> {
    if app.gallery.files.is_empty() { start_gallery_load(app); }
...
pub(crate) fn start_gallery_load(app: &mut AppState) {
    ...
    thread::spawn(move || {
        ...
        if results_dir.exists() {
            let cats = ["imgs", "vids", "audios", "docs", "txts", "codes", "data", "exes", "zips", "unknowns"];
            for cat in cats {
                        ...
                        for entry in entries.flatten() {
                            ...
                            if path.is_file() {
                                files.push(AttachmentFile {
                                    name: path.file_name().and_then(|n| n.to_str()).unwrap_or("unknown").to_owned(),
                                    _path: path.clone(),
...
pub(crate) fn start_analysis(app: &mut AppState) {
    if app.analysis_running { return; }
    app.error = None;
...
pub(crate) fn poll_analysis(app: &mut AppState) {
    if let Some(rx) = &app.analysis_rx {
        ...
        if finished { app.analysis_rx = None; }
    }
}
...
pub(crate) fn handle_download_attachments(app: &mut AppState) {
    if app.download_running { return; }
    if app.last_data.is_none() { try_load_existing_data(app); }
...
pub(crate) fn poll_download(app: &mut AppState) {
    if let Some(rx) = &app.download_rx {
        ...
        if finished { app.download_rx = None; }
    }
}

pub(crate) fn open_selected_support_ticket(app: &mut AppState) {
    if app.support_tickets.as_ref().is_some_and(|t| app.support_ticket_cursor < t.len()) {
...
pub(crate) fn open_selected_channel(app: &mut AppState) -> Result<()> {
    let selected = filtered_channels(app).get(app.channel_cursor).map(|c| (*c).clone());
    if let Some(channel) = selected {
        app.open_message_lines = data::load_message_preview(&channel, app.settings.preview_messages)?;
...
pub(crate) fn filtered_channels(app: &AppState) -> Vec<&data::MessageChannel> {
    app.channel_cache.as_ref().map(|cc| {
        cc.iter().filter(|c| match app.current_filter {
...
pub(crate) fn ensure_channels_loaded(app: &mut AppState) -> Result<()> {
    if app.channel_cache.is_some() { return Ok(()); }
    let package_dir = app.config.package_path(&app.config_path, &app.id);
...
pub(crate) fn try_load_existing_data(app: &mut AppState) {
    if app.last_data.is_some() { return; }
    let results_dir = app.config.results_path(&app.config_path, &app.id);
...
pub(crate) fn is_printable_input(c: char) -> bool { c.is_ascii() && !c.is_ascii_control() }
...
fn activity_event_matches_filters(e: &data::ActivityEventPreview, f: &ActivityFilters) -> bool {
    ...
    if !q.is_empty() && !format!("{} {}", e.summary, e.event_type).to_lowercase().contains(&q) { return false; }
    true
}

pub(crate) fn filtered_gallery_files(app: &AppState) -> Vec<AttachmentFile> {
    if let Some(cat) = &app.gallery.category_filter {
        app.gallery.files.iter().filter(|f| f.category == *cat).cloned().collect()
    } else {
...
pub(crate) fn ratio(p: u64, t: u64) -> f64 { if t == 0 { 0.0 } else { p as f64 / t as f64 } }
pub(crate) fn fmt_num(n: u64) -> String { n.to_string() }
