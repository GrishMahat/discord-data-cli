...
fn level_span(level: u32) -> Span<'static> {
    match level {
        ...
        1 => Span::styled("▪", Style::default().fg(Color::Green).add_modifier(Modifier::DIM)),
...
fn parse_day(s: &str) -> Option<(i64, i32, u32, u32)> {
    ...
    Some((crate::insights::day_ordinal(s)?, y, m, d))
}
...
fn weekday_of(key: &str) -> u32 {
    let ord = crate::insights::day_ordinal(key).unwrap_or(0);
    (((ord + 3) % 7 + 7) % 7) as u32
}
...
fn build_year_grid(by_day: &std::collections::BTreeMap<String, u64>, year: i32) -> YearGrid {
    ...
    let Some((start_ord, _, _, _)) = parse_day(&jan1) else {
        return YearGrid { year, weeks: Vec::new(), total: 0, active_days: 0 };
    };
    ...
    for (key, count) in by_day.iter().filter(|(d, _)| d.starts_with(&year.to_string())) {
        let Some((ord, ..)) = parse_day(key) else { continue };
    ...
    YearGrid { year, weeks, total, active_days }
}
...
impl Levels {
    fn new(values: &mut Vec<u64>) -> Self {
        ...
        Self { thresholds: [q(0.4), q(0.7), q(0.9)] }
    }
...
pub(crate) fn draw_activity_map(frame: &mut ratatui::Frame<'_>, app: &AppState, area: Rect) {
    ...
    for (slot, area) in cols_top.iter().chain(cols_bottom.iter()).enumerate() {
        match shown.get(slot) {
            ...
            None => {
                frame.render_widget(Block::default().borders(Borders::ALL).border_style(Style::default().fg(Color::DarkGray)), *area);
            }
        }
    }
...
fn draw_mini_year(
    ...
    if avail_cols >= 54 && full_weeks <= avail_cols {
        ...
        let start_ord = crate::insights::day_ordinal(&jan1).unwrap_or(0);
        let lead = weekday_of(&jan1) as i64;
        let mut cells: Vec<Option<u32>> = vec![None; full_weeks];
        for m in 1..=12u32 {
            let key = format!("{:04}-{:02}-01", grid.year, m);
            let Some(ord) = crate::insights::day_ordinal(&key) else { continue };
...
mod tests {
    ...
    fn multi_year_wall_renders_all_panels() {
        ...
        for day in [
            "2021-03-07", "2022-06-14", "2023-11-02", "2024-01-05", "2025-09-19",
        ] {
            data.messages
                .temporal
                .by_day
                .insert(day.to_owned(), 42);
        }
    ...
    fn more_than_six_years_paginates() {
        let mut data = AnalysisData::default();
        for y in 2018..=2026 {
            data.messages.temporal.by_day.insert(format!("{y}-06-15"), 7);
        }
        let app = crate::app::test_app_with_data(data);
